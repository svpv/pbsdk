...
void draw_doc(struct terminal *t, struct f_data_c *scr)
{
	...
	if (!scr->vs || !scr->f_data) {
		if (!F) {
			...
			long color = dip_get_color_sRGB(/*0x808080*/ 0xFFFFFF);
...
static int send_to_frame(struct session *ses, struct event *ev)
{
...
#endif
	if ( (r == 1) || (r == 3) ){ //TODO: sbelov original code: if (r == 1) {
		fd->active = 1;
	...
	//if (r == 3) draw_fd_nrd(fd); //TODO: sbelov commented
	if (!F && fd->vs) {
#ifdef JS
