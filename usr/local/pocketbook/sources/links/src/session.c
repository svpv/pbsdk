...
void change_screen_status(struct session *ses)
{
	...
	if (stat) {
		if (stat->state == S_OK)
		{
			...
			update_menu_status(ses->st, 0, stat->state);
		}
		else update_menu_status(NULL, 0, stat->state);
...
/*	  fprintf(stderr,"\nFEFE date %s\n",ce->last_modified); */
	...
	if (stat->state < 0) {
		...
		if (stat->state != S_OK) {
			...
			Message(ICON_INFORMATION, "@Downloading", "@DonwloadOk", 1000);
		}
		abort_download(down);
		return;
	}
...
{
	ses->screen->xp = 0;
	ses->screen->yp = 0; //TODO: sbelov gf_val(1, G_BFU_FONT_SIZE);
	ses->screen->xw = ses->term->x;
	if (ses->term->y < 0/*TODO: sbelov gf_val(2, 2 * G_BFU_FONT_SIZE)*/) ses->screen->yw = 0;
	else ses->screen->yw = ses->term->y /*TODO: sbelov - gf_val(2, 2 * G_BFU_FONT_SIZE)*/;
}
...
static struct session *create_session(struct window *win)
{
	...
	ses->screen->yp = 0; //TODO: sbelov gf_val(1, G_BFU_FONT_SIZE);
	ses->screen->yw = term->y /*TODO: sbelov  - gf_val(2, 2 * G_BFU_FONT_SIZE)*/;
