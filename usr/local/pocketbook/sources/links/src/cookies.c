...
int is_in_domain(unsigned char *d, unsigned char *s)
{
	...
	if (dl <= sl)
	{
		if (dl != sl)
		{
			// if all subdomains
			if (*d == '.')
				result = !casecmp(d, s + sl - dl, dl);

			// try another check if false
			if (!result && s[sl - dl - 1] == '.')
					result = !casecmp(d, s + sl - dl, dl);
		}
		else result = !strcasecmp(d, s);
	}
	return result;
}
...
{
	struct cookie *internal_c;

	free_list(c_domains);

	foreach(internal_c, cookies)
		free_cookie(internal_c);
	free_list(cookies);
