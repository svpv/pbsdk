...
int check_inkview_netstate()
{
	int result = 0;
	int r; //inkview network state

	if (!(QueryNetwork() & NET_CONNECTED) &&
		(r = NetConnect(NULL)) != NET_OK)
	{
		Message(ICON_ERROR, "@Connection", NetError(r), 2000);
		// delete all connections from queue
		abort_all_connections();
		// close browser if we can't connect
		terminate_loop = 1;
	}
	else
	{
		char *proxyUrl = GetProxyUrl();
		if (proxyUrl)
		{
			strncpy(proxies.http_proxy, proxyUrl, MAX_STR_LEN);
			strncpy(proxies.ftp_proxy, proxyUrl, MAX_STR_LEN);
		}
		char *userAgent = GetUserAgent();
		if (userAgent)
			strncpy(http_options.header.fake_useragent, userAgent, MAX_STR_LEN);
		// connection ok
		result = 1;
	}
	return result;
