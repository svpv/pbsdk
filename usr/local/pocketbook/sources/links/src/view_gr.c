...
void g_print_rect(struct graphics_driver *gd, struct graphics_device *device,
int x, int y, int height, int width, long color)
{
  int rect_x = 0;
  int rect_y = 0;
  struct rect clip_reg;
  
  if (y+height<=device->clip.y1||y>=device->clip.y2) return;
  
  rect_x = width;
  rect_y = height;
  
  gd->draw_hline(device, x, y + 1, x+rect_x-1, color);
  gd->draw_hline(device, x, y + rect_y - 2, x+rect_x-1, color);
  gd->draw_vline(device, x, y + 1, y + rect_y - 2, color);
  gd->draw_vline(device, x+rect_x-1, y + 1, y + rect_y - 2, color);
}

void g_print_underline(struct graphics_driver *gd, struct graphics_device *device,
int x, int y, struct style* st, char* text)
{
  int rect_x = 0;
  int rect_y = 0;
  long color = 0x555555;
  ...
  if (y+st->height<=device->clip.y1||y>=device->clip.y2) return;
  
  rect_x = g_text_width(st, text);
  rect_y = st->height;
  
  //color = (style->r1 << 16) | (style->g1 << 8) | (style->b1);
  gd->draw_hline(device, x + 1, y + rect_y - 4, x+rect_x-1, color);
}
	...
	if (link && ((form = link->form)) && ((fs = find_form_state(fd, form)))) {
		switch (form->type) {
			...
			case FC_RADIO:
				...
				g_print_text(drv, fd->ses->term->dev, x + (fs->state ? (inv->height - g_text_width(inv, "\u2981"))/2 : 0), y, inv, fs->state ? "\u2981" : "  ", NULL);
				g_print_rect(drv, fd->ses->term->dev, x, y, inv->height, inv->height, (inv->r1 << 16) | (inv->g1 << 8) | (inv->b1));
				if (in) g_free_style(inv);
				return;
			case FC_CHECKBOX:
				if (link && fd->active && fd->vs->g_display_link && fd->vs->current_link == link - fd->f_data->links) inv = g_invert_style(t->style), in = 1;
				else inv = t->style, in = 0;
				g_print_text(drv, fd->ses->term->dev, x + (fs->state ? (inv->height - g_text_width(inv, "\u06f7"))/2 : 0), y, inv, fs->state ? "\u06f7" : "  ", NULL);
				g_print_rect(drv, fd->ses->term->dev, x, y, inv->height, inv->height, (inv->r1 << 16) | (inv->g1 << 8) | (inv->b1));
				if (in) g_free_style(inv);
				return;
			case FC_SELECT:
				...
				g_print_text(drv, fd->ses->term->dev, x, y, inv, form->labels[fs->state], NULL);
				while (l < t->xw){
					g_print_text(drv, fd->ses->term->dev, x + l, y, inv, " ", &l);
					padding++;
				}
				text_width = g_text_width(inv, form->labels[fs->state]) + padding*g_text_width(inv, " ");
				g_print_rect(drv, fd->ses->term->dev, x, y, inv->height, text_width, (inv->r1 << 16) | (inv->g1 << 8) | (inv->b1));
				g_print_text(drv, fd->ses->term->dev, x + text_width - inv->height + (inv->height - g_text_width(inv, "\u2207"))/2, y, inv, "\u2207", NULL);
				g_print_rect(drv, fd->ses->term->dev, x + text_width - inv->height, y, inv->height, inv->height, (inv->r1 << 16) | (inv->g1 << 8) | (inv->b1));
				if (in) g_free_style(inv);
				return;
			case FC_TEXT:
			case FC_PASSWORD:
			case FC_FILE:
				...
				*/
				...
				g_print_rect(drv, fd->ses->term->dev, x, y, t->style->height, t->xw, (t->style->r1 << 16) | (t->style->g1 << 8) | (t->style->b1));
				return;
			case FC_TEXTAREA:
				...
				mem_free(lnx);
				g_print_rect(drv, fd->ses->term->dev, x, y, t->style->height * form->rows, t->xw, (t->style->r1 << 16) | (t->style->g1 << 8) | (t->style->b1));
				return;
		}
	}
...
#define B_ABOVE(t, m) (highlight_positions[t] >= end)
	...
	if (link && ((form = link->form)) /*&& (FC_BUTTON == form->type)*/)
	{
		text_width = g_text_width(t->style, " ");
		g_print_rect(drv, fd->ses->term->dev, x, y , t->style->height, t->xw, (t->style->r1 << 16) | (t->style->g1 << 8) | (t->style->b1) );		
	} else if ( link )
	{
		g_print_underline(drv, fd->ses->term->dev, x, y, t->style, t->text);
	}
}
	...
	switch (ev->ev) {
		case EV_MOUSE:
			...
			if ((ev->b & BM_ACT) == B_DOWN && fd->f_data->vsb && ev_in_rect(ev, fd->xw - G_SCROLL_BAR_WIDTH, 0, fd->xw, fd->yw - fd->f_data->hsb * G_SCROLL_BAR_WIDTH)) {
				...
				return 3; //TODO: sbelov previous code: break;
			}
