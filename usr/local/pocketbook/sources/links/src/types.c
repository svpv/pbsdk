...
void create_initial_extensions(void)
{
	...
	ext.ext="fb2",ext.ct="application/fb2",update_ext(&ext);
	...
	ext.ext="txt",ext.ct="application/txt",update_ext(&ext);
...
static unsigned char *get_content_type_by_extension(unsigned char *url, unsigned char *head)
{
	struct extension *e;
	struct assoc *a;
	...
	foreach(e, extensions) {
		...
		// If ext is php, try to detect file extension by header
		if (!fname && head && (extl == 3 && !casecmp(ext, "php", 3)))
			fname = get_filename_from_header(head);
		if (!fname) {
...
unsigned char *get_content_type(unsigned char *head, unsigned char *url)
{
	...
	if ((ct = parse_http_header(head, "Content-Type", NULL))) {
		...
		if (!strcasecmp(ct, "text/plain") || !strcasecmp(ct, "application/octet-stream") || !strcasecmp(ct, "application/octetstream") || !strcasecmp(ct, "application/octet_stream")) {
			unsigned char *ctt = get_content_type_by_extension(url, head);
			if (ctt) {
				mem_free(ct);
				return ctt;
			}
	...
	ct = get_content_type_by_extension(url, head);
	if (ct) return ct;
