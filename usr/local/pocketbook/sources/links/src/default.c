...
unsigned char *get_home(int *n)
{
	...
	unsigned char *config_dir = NULL;//stracpy("links"/*LINKS_HOME*/);

	if (n) *n = 1;
#ifdef WIN32
	if (!home) home = stracpy(getenv("APPDATA"));
#endif
	if (!home) home = stracpy(/*getenv("HOME")*/ USERDATA);
	if (!home
#ifdef WIN32
...
struct document_setup dds = { 0, 0, 1, 1, 0, 0, 3, 0, 0, 0, 24, 1, 
	100, /* Image scale */
	0, /* Porn enable */
	0 };

struct rgb default_fg = { 191, 191, 191, 0 };
//struct rgb default_fg = { 0xFF, 0xFF, 0xFF, 0 };
...
struct rgb default_bg_g = { 0xFF, 0xFF, 0xFF, 0 };
...
int js_manual_confirmation=0; /* !0==annoying dialog on every goto url etc. */
...
struct option links_options[] = {
	...
	{1, unset_cmd, NULL, NULL, 0, 0, &bookland_on, NULL, "bookland-off"},
